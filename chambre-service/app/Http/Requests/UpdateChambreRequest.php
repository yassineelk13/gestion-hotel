...
class UpdateChambreRequest extends FormRequest
{
    ...
    {
        $chambreId = $this->route('id'); // Récupérer l'ID de la route
        
        return [
            // Optionnels (tous avec 'sometimes')
            'numero' => 'sometimes|string|max:10|unique:chambres,numero,' . $chambreId . ',id_chambre',
            'prix_par_nuit' => 'sometimes|numeric|min:0',
            'type' => 'sometimes|string|max:50',
            'capacite_personne' => 'sometimes|integer|min:1|max:10',
            'nb_lits' => 'sometimes|integer|min:1|max:5',
            'superficie' => 'sometimes|numeric|min:0|max:200',
            'etage' => 'sometimes|integer|min:0|max:20',
            'vue' => 'sometimes|string|max:100',
            'description' => 'sometimes|string|max:500',
            'photo_url' => 'sometimes|url|max:255',
            'statut' => 'sometimes|in:libre,occupee,maintenance,hors_service',
        ];
    }
